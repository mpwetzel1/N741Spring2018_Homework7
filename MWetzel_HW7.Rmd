...
# Fit on rpart model with mcs as outcome and cesd as predictor
...
# Put code here for Problem 2# Melt the other variables down and link to cesd
h1m <- reshape2::melt(h1a, id.vars = "mcs") # Per the Internet, melt just reshapes the data from wide to long
...
mcs_lr <- glm(mcs_lt45 ~ age + female + pss_fr + homeless + 
              pcs + cesd, data = h1)
summary(mcs_lr)

# Visually compare models
stargazer(glm1, mcs_lr, title="Comparison of MCS and CESD Models",
          type = "text", align=TRUE)

```

In terms of the predictors, age, homeless, and perceived social support are not significant in either model. Physical health scale is a significant predictor of dpression, but not for general mental heath.
...
summary(fitk)
# Plot the tree
plot(fitk, uniform = TRUE, compress = FALSE,
...
mcs_bin <- rpart::rpart(mcs_lt45 ~ age + female + pss_fr + 
                       homeless + pcs + cesd, 
                     method = "class", data = h1)
class(mcs_bin)
# Display the results
rpart::printcp(mcs_bin)
#Visualize the cross-validation results 
rpart::plotcp(mcs_bin)
# Get a detailed summary of the splits
summary(mcs_bin)
# Plot the tree
plot(mcs_bin, uniform = TRUE, 
     main = "Classification Tree for MCS < 45")
text(mcs_bin, use.n = TRUE, all = TRUE, cex = 0.8)
...
# female, pss_fr, homeless, pcs, mcs
LowMH <- rpart::rpart(mcs_lt45 ~ age + female + 
...
The code for this assignment can be found on [Github](https://github.com/mpwetzel1/N741Spring2018_Homework7)
